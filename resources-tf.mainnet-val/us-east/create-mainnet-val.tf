...
locals {
        vm_profile      = "bx2-4x16"
        zones           = 3
	vm_list		= ["node11","node12","node13","node0"]
}
...
resource "ibm_is_instance" "vm1" {
	...
	keys	= [ibm_is_ssh_key.rchain-owner.id]
        tags	= [ "mainnet", "validator" ]
	boot_volume  { name = "${var.resource_group}-${var.region}-${local.vm_list[count.index]}-boot" }
	resource_group = data.ibm_resource_group.res_grp.id
...
resource "ibm_is_volume" "vol1" {
	count	= length(local.vm_list)
	name	= "${var.resource_group}-${var.region}-${local.vm_list[count.index]}-data"
	profile	= "5iops-tier"
	zone	= "${var.region}-${count.index % local.zones + 1}"
        tags	= [ "mainnet", "validator" ]
	capacity= 1000
...
resource "ibm_is_floating_ip" "fip1" {
	count	= length(local.vm_list)
	name	= "${var.resource_group}-${var.region}-${local.vm_list[count.index]}-fip"
