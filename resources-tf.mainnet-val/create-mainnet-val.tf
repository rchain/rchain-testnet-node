...
resource "ibm_is_instance" "vm1" {
	...
	keys	= [ibm_is_ssh_key.rchain-owner.id]
        tags	= [ "mainnet", "validator" ]
	boot_volume  { name = "${var.resource_group}-${var.region}-${var.vm_list[count.index]}-boot" }
	resource_group = data.ibm_resource_group.res_grp.id
...
resource "ibm_is_volume" "vol1" {
	count	= length(var.vm_list)
	name	= "${var.resource_group}-${var.region}-${var.vm_list[count.index]}-data"
	profile	= "5iops-tier"
	zone	= "${var.region}-${count.index % local.zones + 1}"
        tags	= [ "mainnet", "validator" ]
	capacity= 1000
...
resource "ibm_is_floating_ip" "fip1" {
	count	= length(var.vm_list)
	name	= "${var.resource_group}-${var.region}-${var.vm_list[count.index]}-fip"
