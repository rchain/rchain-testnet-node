...
data "ibm_security_group" "allow_in_multi_rnode"{ name = "allow_in_multi_rnode"}
...
locals { 
   ...
   region       = "eu-gb" 
   datacenter   = "lon06" 
   domain_name  = "dev.rchain.coop"
}

# Finally create some servers
resource "ibm_compute_vm_instance" "sandbox" {
  hostname                 = "sandbox"
  domain                   = local.domain_name
  flavor_key_name          = "M1_8X64X100"
  ...
  public_security_group_ids  = [data.ibm_security_group.allow_in_rnode2.id,
				data.ibm_security_group.allow_in_multi_rnode.id,
